...
<body>
   ...
   <div class="container">
        <h1>와인 수정</h1>
        <form action="${contextPath}/admin/updateWine/${wine.wineNo}" id="updateWine" method="post" enctype="multipart/form-data">
            
            <label for="wineNo">와인 번호:</label>
            <input type="number" id="wineNo" name="wineNo" readonly="readonly" style="background-color: lightgray" required>
            ...
            <select name="grapeNo" id="grapeNo" required>
            </select>
            ...
            <label>이미지 파일: ${wine.wineImage.originName}</label>
    ...
    <script>
		...
		function selectWineType(callback) {
			...
			$.ajax({
	    		...
	    		success : (gList)=>{
	                ...
	                $("#grapeNo").val(${wine.grape.grapeNo});
	    		},
	    		error : (xhr)=>{
	    			$("#grapeNo").empty();
	    			console.log(xhr);
		...
		(()=>{
			$("#wineNo").val(${wine.wineNo})
            $("#wineName").val(`${wine.wineName}`)
            $("#wineTypeNo").val(${wine.wineType.wineTypeNo})
            
            selectWineType();
			
			$("#countryNo").val(${wine.country.countryNo})
            $("#body").val(${wine.characteristic.body})
            $("#sweet").val(${wine.characteristic.sweet})
            $("#bitter").val(${wine.characteristic.bitter})
            $("#sour").val(${wine.characteristic.sour})
            $("#vintage").val(${wine.vintage})
            $("#price").val(${wine.price})
            $("#quantity").val(${wine.quantity})
            $("#content").val(`${wine.content}`)
            $("#status").val(`${wine.status}`)
